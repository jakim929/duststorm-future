...
erosion_count = intarr(117,111)
average_tust = fltarr(117,111)
total_SSI = intarr(117,111)
all_SSI = intarr(117,111)
vdf_yearly = fltarr(117,111)
vdf_count = fltarr(117,111)
...
vdf_daily = fltarr(117,111,365)
hdf_daily = fltarr(117,111,365)

point_vdf = fltarr(365)
point_ust = fltarr(365)
point_tust = fltarr(365)
point_smois = fltarr(365)
dxstart = 38
dxend = 45
dystart = 91
dyend = 95
...
for m = 1, 12 do begin
    ...
    erosion_monthly = intarr(117,111)
    for d = 1, nday(m-1) do begin
    	
    	
    	  erosion_daily = intarr(117,111)
        SSI_temp = intarr(117,111)
        tot_temp = intarr(117,111)
        ...
        vdf_temp = fltarr(117,111)
        hdf_temp = fltarr(117,111)
            ...
            ncdf_close, fid
                ...
                for dx = 0, 116 do begin
                    for dy = 0, 110 do begin
                    	...
                    	t_tust = ((0.0123 * ((t_pd/t_ad)*9.8*t_gsavg + (5.0*10^(-4))/(t_ad* t_gsavg)))^0.5) * t_rcf * t_mcf	
                    	;threshold u* can be 1.65 - 3
                    	
                    	average_tust(dx,dy) = average_tust(dx,dy) + t_tust
                    	...
                    	case t_clay of ; Marticorena Bergametti 1995, Tegen et al 2001 - Astitha 2012 under 20micrometers
                    		...
                    		(t_clay ge 20) and (t_clay lt 45): t_a = 10.^(-4)
                    		else: t_a = 10.^(-5)
                    	endcase
                    	...
                    	if t_vdf ne 0 then begin
                    		vdf_yearly(dx,dy) = vdf_yearly(dx,dy) + t_vdf
                    		vdf_count(dx,dy) = vdf_count(dx,dy) + 1.0
                    	...
                    	if t_vdf gt vdf_temp (dx,dy) then vdf_temp(dx,dy) = t_vdf
                    	
                    	if t_hdf gt hdf_temp (dx,dy) then hdf_temp(dx,dy) = t_hdf
                    	...
                    	;if t_lm eq 1 and t_wpara ge 8. and t_vdf ge 40.07*10.^(-7) and t_hdf ge 255.67*10.^(-4)and t_ust gt t_tust then begin
                            ...
                            if t_smois gt smois_max(dx,dy) then smois_max(dx,dy) = t_smois
                                 
                                 if t_vdf ge 0.7 * 10.^(-7) and t_hdf ge 0.7*10.^(-4) then begin
        ...
        erosion_monthly = erosion_monthly + erosion_daily
        total_SSI = total_SSI + tot_temp
		
		ust_daily(*,*,dayCount-1) = ust_temp(*,*)
		vdf_daily(*,*,dayCount-1) = vdf_temp(*,*)
		hdf_daily(*,*,dayCount-1) = hdf_temp(*,*)
		dayCount = dayCount + 1
		...
		for xst = dxstart, dxend do begin
			for yst = dystart, dyend do begin
			  ...
			  if smois_max(xst,yst) ne 1.0 then begin
				...
				if smois_max(xst,yst) ne 0.0 then begin
					daySmois = daySmois + smois_max(xst,yst)
					realCountSmois = realCountSmois + 1
				endif  
		...
		if (realCountSmois ne 0) then begin
		  smoisDailyAvg = daySmois / realCountSmois
		endif else begin  
			smoisDailyAvg = 0
		endelse
		...
		point_smois(startDay-1) = smoisDailyAvg
		startDay = startDay + 1
    ...
    index = where(erosion_monthly ne 0, count)
    ;print, max(hdf_ms), max(vdf_ms)
    if count ne 0 then begin
       vdf_ms(index) = vdf_ms(index)/erosion_monthly(index)
       hdf_ms(index) = hdf_ms(index)/erosion_monthly(index)
...
vdf_yearly = vdf_yearly / vdf_count

average_tust = average_tust/loopcount
all_SSI = SSI_mon(*,*,0) + SSI_mon(*,*,1)+ SSI_mon(*,*,2)+ SSI_mon(*,*,3)+ SSI_mon(*,*,4)+ SSI_mon(*,*,5)+ SSI_mon(*,*,6)+ SSI_mon(*,*,7)+ SSI_mon(*,*,8)+ SSI_mon(*,*,9)+ SSI_mon(*,*,10)+ SSI_mon(*,*,11)

;write_tiff, '/home2/sunwoo/mask.tif', t_lm, /long


;write_tiff, '/home2/sunwoo/foutput4/SSI_output_current.tif', SSI_mon, /long
;write_tiff, '/home2/sunwoo/foutput4/hdf_output_current.tif', hdf_avg, /float
;write_tiff, '/home2/sunwoo/foutput4/vdf_output_current.tif', vdf_avg, /float
;write_tiff, '/home2/sunwoo/foutput4/wind_output_current.tif', windv_avg, /float
;write_tiff, '/home2/sunwoo/foutput4/smois_output_current.tif', smois_avg, /float
;write_tiff, '/home2/sunwoo/foutput4/ust_output_current.tif', ust_avg, /float
;write_tiff, '/home2/sunwoo/foutput4/tust_output_current.tif', tust_avg, /float
;write_tiff, '/home2/sunwoo/foutput4/ust_dailyoutput_current.tif', ust_daily, /float
;write_tiff, '/home2/sunwoo/foutput4/tot_SSI_current.tif', total_SSI, /float
;write_tiff, '/home2/sunwoo/foutput4/all_SSI_current.tif', all_SSI, /float
;write_tiff, '/home2/sunwoo/final_output/foutput1/vdf_yearly_2081.tif', vdf_yearly, /float

write_csv, '/home2/sunwoo/point_output/point_vdf_future(lks).csv', point_vdf
write_csv, '/home2/sunwoo/point_output/point_ust_future(lks).csv', point_ust
write_csv, '/home2/sunwoo/point_output/point_tust_future(lks).csv', point_tust
write_csv, '/home2/sunwoo/point_output/point_smois_future(lks).csv', point_smois
;write_tiff, '/home2/sunwoo/foutput4/erosion_count_current.tiff', erosion_count,/float
;write_tiff, '/home2/sunwoo/foutput4/average_tust_2001.tif', average_tust, /float
end