...
data = read_tiff('/home2/sunwoo/ust_dailyoutput_current.tif') ; read the MP LAI outputs 
;data = read_tiff('/home2/sunwoo/hdf_output_current.tif') ; read the MP LAI outputs 
;data = reverse(data,2)
...
Tmax = 1.0; Setting Coef. maximum value
Tmin = 0.0 ; Setting Coef. minimum value
...
Erase, color=fsc_color('white') ; Setting the background color
...
LoadCT, 0, /Silent ; Loading the color table for the background
   ...
   colorbar, NCOLORS=253, POSITION=loc, divisions=5, /vertical, color='black', $; Drawing the color bar
             charsize=1.5, minrange=Tmin, maxrange=Tmax, /right, charthick=2, format='(F7.1)'
   ...
   XYOutS, 0.5, 0.915, 'U* (m/s) for '+String(i+1)+' , 2001', font=1, charsize=4, Color=FSC_Color('black'), $ ; Putting the Title
         /Normal, Alignment=0.5
      snapshot=tvrd(true=1)
   Write_png, '/home2/sunwoo/ust_'+String(i+1)+'_01.png', snapshot ; Save the pop-up window image
   print, String(i+1)
endfor
end
