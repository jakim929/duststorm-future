...
;data = read_tiff('/home2/sunwoo/SSI_output_future.tif') ; read the MP LAI outputs 
data = read_tiff('/home2/sunwoo/foutput0/SSI_output_current.tif') ; read the MP LAI outputs 
;data = reverse(data,2)
...
input = fltarr(dx,dy) ; Making a new array for drawing

Tmax = 31 ; Setting Coef. maximum value
...
DEVICE, SET_FONT='Helvetica', /TT_FONT ; Setting the font type
   ...
   Image_position1=[0.05, 0.05, 0.85, 0.85] ; setting the image position
   loc=[0.91, 0.05, 0.95,0.84] ; setting the legend position
...
LoadCT, 0, /Silent ; Loading the colobr table for the background
   ...
   XYOutS, 0.5, 0.915, 'Dust storm possibility frequency for '+nmon(i)+', 2001', font=1, charsize=4, Color=FSC_Color('black'), $ ; Putting the Title
         /Normal, Alignment=0.5
      snapshot=tvrd(true=1)
   ;Write_png, '/home2/sunwoo/final_graphs/2081_1/DSF_'+ nmon(i) +'_2081.png', snapshot ; Save the pop-up window image
endfor
end
