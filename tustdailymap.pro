...
data = read_tiff('/home2/sunwoo/tust_output_current.tif') ; read the MP LAI outputs 
...
Tmax = 0.02 ; Setting Coef. maximum value
Tmin = 0.005 ; Setting Coef. minimum value
...
LoadCT, 0, /Silent ; Loading the color table for the background
   ...
   XYOutS, 0.5, 0.915, 'Threshold U* (m/s) for '+nmon(i)+', 2001', font=1, charsize=4, Color=FSC_Color('black'), $ ; Putting the Title
         /Normal, Alignment=0.5
      snapshot=tvrd(true=1)
   Write_png, '/home2/sunwoo/tust_'+nmon(i)+'_01.png', snapshot ; Save the pop-up window image
endfor
end
